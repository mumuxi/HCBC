...
//			System.out.println(list.size());
			if (list != null) {
				...
				for (int n = 0; n < listSize; n++) {
					...
					Vector<Integer> interSet = FCBC_GIC
							.intersect(vectData, attrVector);
					Vector<Integer> unionSet = FCBC_GIC.union(
							vectData, attrVector);
					if (FCBC_GIC.relationOf2Vector(attrVector, vectData) == 0) {
						return concept;
					}
...
//			System.out.println(list.size());
			if (list != null) {
				...
				for (int n = 0; n < listSize; n++) {
					...
					Vector<Integer> attrVector = concept.getAttrSet();
//					/*
					if(attrVector.size() < 1 || attrVector.get(attrVector.size() - 1) < NoClass_FCBC.point)
						continue;
//					*/
					Vector<Integer> interSet = FCBC_GIC
							.intersect(vectData, attrVector);
...
//					/*
					if(attr.size() < 1 || attr.get(attr.size() - 1) < NoClass_FCBC.point)
						continue;
//					*/
					Vector<Integer> attrInterSet = FCBC_GIC
							.intersect(attrVect, attr);
					Vector<Integer> objInterSet = FCBC_GIC
							.intersect(objVect, obj);
					if(attr.size() == 0 || obj.size() == 0)
						continue;
					if (FCBC_GIC.relationOf2Vector(attr, attrVect) == 0) {
						return concept;
					}
					/*
					 * �������Բ���
		...
		for (Iterator<List<Concept>> iter = concepts.values().iterator(); iter.hasNext();) {
			...
			if (list != null) {
				...
				for(int index = 0 ;index < listSize;index++) {
					...
					int attrVectSize = attrVect.size();
//					/*
					if(attrVectSize < 1 || attrVect.get(attrVectSize - 1) < NoClass_FCBC.point)
						continue;
//					*/
					double[] attrsWeight = new double[attrVectSize + 1];
					for(int i = 0;i < attrVectSize + 1;i++) {
...
//					*/
					...
					if(attrVect.get(attrVectSize - 1) < NoClass_FCBC.point) {
...
//			sumWeight += 1.0 * attrHeight / (attrHeight + tempValue - 1);//4
			tempD = FCBC_LOOCV.weightFun3(tempValue, attributeHeight.get(tempAttr), bot, conceptCount, attrsCount[tempAttr]);
//			divider1 += tempD * tempD;
			divider1 += tempD;
...
//			System.out.println(dividend + " " + divider1 + " " + divider2 + " " + tempSim);
		...
		if(resultMap.size() < NoClass_FCBC.NUM_OF_CLUSTER) {
...
//		System.out.println(bottomConcept);
		...
		for(int i = 0;i < firstPSize;i++) {
			...
			Vector<Integer> interSet = FCBC_GIC.intersect(parentAttr, rowData);
...
//		System.out.println(max + "  " + listRemain.size());
		...
		for(int i = 0;i < firstPSize;i++) {
			...
			for(int j = 0;j < listRemain.size();j++) {
				...
				Vector<Integer> interSet = FCBC_GIC.intersect(iSet, subAttr);
				int interSize = interSet.size();
				if(interSize == iSize) {
					flag = false;
					break;
				...
				if(interSize < iSize) {
					if(FCBC_GIC.intersect(rowData, subAttr).size() > interSize)
						continue;
					listRemain.remove(j);
					j--;
				}
...
//		System.out.println(bParent.size() + "  " + listRemain.size());
		...
		for(int i = 0;i < listRemain.size();i++) {
			...
			Concept parentConcept = concepts.get(pair.getIndexOfMap()).get(pair.getIndexOfBucket());
			int sizez = FCBC_GIC.intersect(parentConcept.getAttrSet(), rowData).size();
			if(sizez == 0)
				continue;
...
//					int totalNum = context.length;
					...
					int attrVectSize = attrVect.size();
//					/*
					if(attrVectSize < 1 || attrVect.get(attrVectSize - 1) < NoClass_FCBC.point || concept.getObjSet().size() == 0)
						continue;
//					*/
					//��ʼ�������ƶ�
